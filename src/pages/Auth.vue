...
async function handleLogin() {
  if (loginType.value === 'email') {
    ...
    if (emailError)
      return message.warning(emailError)

    const passwordError = validatePassword(loginForm.value.emailPassword)
    if (passwordError)
      return message.warning(passwordError)
    ...
    if (usernameError)
      return message.warning(usernameError)
...
async function confirmCaptcha() {
  ...
  try {
    loading.value = true
    message.loading('正在登录，请稍候...', { duration: 0 })
    let res
    ...
    message.destroyAll() // ✅ 清除所有 loading
    loading.value = false
  }
}
...
async function handleRegister() {
  const { username, email, verificationCode, password, confirm } = registerForm.value
  ...
  if (codeError)
    return message.warning(codeError)
  ...
  if (confirmError)
    return message.warning(confirmError)
    ...
    <div class="anth-inner">
      ...
      <div class="card-container" :class="{ 'show-register': !isLogin }">
        ...
        <div class="login-box card-panel">
          ...
          <n-button color="#C10200" :disabled="!agreed" round class="p-btn" :loading="loading" @click="handleLogin">
            登录
          </n-button>
          <n-divider>
            其他登录方式
          ...
          <n-checkbox v-model:checked="agreed" class="else-btn">
            我已阅读并同意
        ...
        <div class="register-box card-panel">
          <div class="register-inner">
            ...
            <n-button color="#C10200" round class="p-btn" :loading="loading" @click="handleRegister">
              注册
            </n-button>
    ...
    >
      ...
      <template #default>
        <div class="captcha-body">
          ...
          <n-input v-model:value="captchaInput" placeholder="" class="captcha-input" @keyup.enter="throttledConfirmCaptcha" />
          <div
      ...
      <template #action>
        <div class="captcha-footer">
          <n-button size="small" @click="cancelCaptcha">
            取消
          </n-button>
          <n-button size="small" type="primary" class="p-btn" @click="throttledConfirmCaptcha">
            确认
          </n-button>
        </div>
      </template>
